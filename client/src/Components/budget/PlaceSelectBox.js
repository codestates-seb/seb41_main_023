...
const PlaceSelectBox = ({ data, setPlaceCategory, handlePlace, planDateId }) => {
  ...
  return (
    <ModalContainer onClick={() => setPlaceCategory(false)}>
      <ModalWrapper onClick={(e) => e.stopPropagation()}>
        <div className="title_frame">
          <div className="title">Select Place</div>
          <div className="cancel_button" onClick={() => setPlaceCategory(false)}>
            ❌
          </div>
        </div>
        <div className="category_list">
          {places.map((el) => (
            <div
              ...
              onClick={() => handlePlace(el.placeName)}
            >
              {el.placeName}
            </div>
          ))}
