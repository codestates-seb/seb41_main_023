...
const Explore = (props) => {
        ...
        const fetchMoreExplores = useCallback(async () => {
            await axios
                .get(`${process.env.REACT_APP_API_URL}/board?page=${page.current}&size=5&tab=likes`, {
                    headers: {
                        Authorization: token,
                    },
                })
        ...
        useEffect(() => {
                if (props.searches) {
                    axios.get(`${process.env.REACT_APP_API_URL}/board?page=1&size=100&tab=boardId&city=${props.destination}`, {
                        headers: {
                            Authorization: token,
                        },
                    })
                    ...
                    if(window.location.pathname === '/board') {
                        ...
                        axios.get(`${process.env.REACT_APP_API_URL}/board?page=1&size=5&tab=likes`, {
                            headers: {
                                Authorization: token,
                            },
                        })
            ...
            <ExploreContainer>
                <div className="contents">
                    {exploreList.length !== 0 ?
                        ...
                        ))) : <div className={'search__error'}>검색어와 일치하는 게시글이 없습니다</div>}
